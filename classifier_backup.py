...
class EnhancedPEFundClassifier:
    ...
    def _apply_fund_special_rules(self, fund_name: str, fund_lower: str) -> Tuple[float, List[str]]:
        ...
        # Strong acquisition vehicle indicators
        acquisition_terms = ['holdco', 'topco', 'bidco', 'acquico', 'newco']
        found_acquisition_terms = [term for term in acquisition_terms if term in fund_lower]
        if found_acquisition_terms:
            acquisition_bonus = len(found_acquisition_terms) * 2.0  # 2.0 per term
            total_score += acquisition_bonus
            reasons.append(f'Acquisition Vehicle Indicators: {", ".join(found_acquisition_terms)} (+{acquisition_bonus:.1f})')
